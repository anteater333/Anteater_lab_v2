...
- `useMemo()`는 JS 관점에서 연산 시간이 오래 걸리는 값을 메모할 때에 사용하자.
...
React 소스 코드를 훑어보면서 한 가지 유추할 수 있는 사실은, React 프레임워크 내부에서도 `memo()`로 만든 컴포넌트는 꽤나 특별한 위치에 있는 것 같다는 점이다. `memo()` 만든 컴포넌트는 리액트 18버전 기준 단 13개만이 존재하는 Symbol 중 하나를 부여받는다. 리액트 코어 모듈 안에서 컴포넌트를 처리하는 로직은 이 심볼을 인식해 로직의 분기를 나눌 것이다.

아무튼 이렇게 복잡하게 파헤쳐봤지만 결국 정답은 간단하다. 컴포넌트를 메모할 땐 `React.memo()`를 씁시다.
...
그런데 싹다 메모하는게 "정말로" 해로운가?
-> 그래서 코드 컨벤션으로 그냥 모두 메모하는걸로 정해둔 팀도 있다고 카더라
-> 코드 가독성은 어떻게 할건데?

내노라하는 개발자들도 한바탕 싸운 마당에 내가 정답을 내기는 어렵겠지만,
최소한 판단의 근거는 마련한 메모이제이션이 되어야 한다.

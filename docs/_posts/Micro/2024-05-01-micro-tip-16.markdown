...
지금부터는 민망한 내 과거 시도를 되살펴보자. 내 생각은 이랬다. `const MyButton = <button>{ myValue }</button>` 같은 표현식. 상수값, 배열, 함수, 객체 등등 여타 다른 값들 처럼 사용 가능한 JSX 문법의 엘리먼트 표현식을 `useMemo`로 메모함으로써 (혹은 `useCallback`으로 함수헝 컴포넌트 형태로 메모함으로써) 컴포넌트를 메모할 수 있지 않을까? 사실 역사를 따지면 `React.memo()`가 `useMemo()`보다 먼저 나온 기술이지만, 애석하게도 나는 훅 밖에 몰랐다. 나의 리액트에 대한 학습은 On-demand 방식으로 이루어지고 있었는데 (고상하게 말하면 이렇고, 그냥 필요한게 있다 싶을 때마다 구글과 SO와 GPT의 도움을 받는 식) 마침 메모이제이션에 대한 수요가 생겼고, 내 머릿 속 캐시 서버 안에는 메모이제이션이라는 키워드는 `useMemo()`와 매핑되어 있었고... 이래서 사람이 교재를 보면서 차근차근 배워야 하는거야.

가장 큰 문제는, 그걸 구현해본 코드가 겉으로 보기에는 마치 의도대로 동작하는 것 처럼 보인다는 점이다.
